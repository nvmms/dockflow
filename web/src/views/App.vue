  ...
  <section class="resource-page">
    ...
    <el-card shadow="never" class="resource-card">
      ...
      <el-table :data="filtered" v-loading="loading" empty-text="当前命名空间还没有应用">
        ...
        <el-table-column align="right" width="350"><template #default="{ row }"><el-button link @click="openEdit(row)">编辑</el-button><el-button link type="primary" @click="openDeploy(row)">部署</el-button><el-button link @click="openBuildLogs(row)">打包日志</el-button><el-button link :disabled="!row.deploy?.length" @click="openLogs(row)">运行日志</el-button><el-button link type="danger" @click="remove(row)">删除</el-button></template></el-table-column>
      </el-table>
    </el-card>
  </section>
...
async function openBuildLogs(row:AppRecord){selected.value=row;buildLogsDialog.value=true;buildLogsLoading.value=true;try{deploymentJobs.value=(await api.get<DeploymentJob[]>(`/namespaces/${props.namespace}/apps/${row.name}/deploy/jobs`)).sort((a,b)=>b.startedAt.localeCompare(a.startedAt));selectedJobId.value=deploymentJobs.value[0]?.id||'';currentJob.value=deploymentJobs.value[0];if(currentJob.value?.status==='running')startPolling()}catch(e){ElMessage.error((e as Error).message)}finally{buildLogsLoading.value=false}}
...
function openLogs(row:AppRecord){selected.value=row;selectedContainer.value=row.deploy?.at(-1)?.containerId||'';logs.value='';logsDialog.value=true;loadLogs()}
