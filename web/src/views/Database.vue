  ...
  <section class="resource-page">
    ...
    <el-card shadow="never" class="resource-card"><div class="table-toolbar"><span class="record-count">{{ records.length }} 个实例</span><el-button text :loading="loading" @click="load">刷新</el-button></div>
      <el-table :data="records" v-loading="loading" empty-text="当前命名空间还没有数据库">
        ...
        <el-table-column align="right" width="240"><template #default="{row}"><el-button link @click="exportSQL(row)">导出</el-button><el-button link @click="chooseImport(row)">导入</el-button><el-button link type="danger" @click="remove(row)">删除</el-button></template></el-table-column>
...
import { reactive, ref, watch } from 'vue';import { ElMessage,ElMessageBox } from 'element-plus';import { api,type DatabaseRecord } from '../api'
const props=defineProps<{namespace:string}>();const records=ref<DatabaseRecord[]>([]),loading=ref(false),dialog=ref(false),saving=ref(false),fileInput=ref<HTMLInputElement>(),importTarget=ref<DatabaseRecord>();const form=reactive({name:'',dbtype:'mysql:5.7',dbname:'',username:'',password:'',remote:false,cpu:1,memory:2})
async function load(){loading.value=true;try{records.value=await api.get<DatabaseRecord[]>(`/namespaces/${props.namespace}/databases`)||[]}catch(e){ElMessage.error((e as Error).message)}finally{loading.value=false}}
